...
move => Hc Ha.
by rewrite [f b _] Hc [f c _] Hc -Ha.
Restart.
...
rewrite /maxn.
case: (leqP m n)=>//;
case: (leqP (m.+1)  (n.+1)) =>//; move=> H H'; f_equal.
- rewrite ltnS in H.
  suff : False => //.
  apply huh' with (n:= n)(m := m); by split.
- rewrite ltnS in H.
  suff : False => //.
  apply huh' with (n:= n)(m := m); by split.
Restart.
...
(* my proof *)
rewrite /maxn.
case: (leqP (p + m)  (p+n)) => // => Hc; congr (_+_).
- rewrite leq_add2l in Hc.
  apply leq_gtF in Hc.
  by rewrite Hc.
- rewrite ltn_add2l in Hc.
  by rewrite Hc. 
Restart.
...
(* my proof *)
rewrite /minn /maxn.
by case: natrelP => // => H; rewrite addnC.
Qed.
