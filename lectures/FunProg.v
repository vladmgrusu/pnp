...
Fail Fixpoint my_plus_buggy n m := 
...
Fixpoint alternate (l1 l2 : seq nat) : seq nat :=
  match l1, l2 with
  | a :: l'1, b :: l'2 => a :: b :: (alternate l'1 l'2)
  | l, nil => l
  | nil, l => l
  end.    
...
Fixpoint numerator (r:rational) : nat :=
  match r with
  |one => 1
  |N x => denominator x + numerator x
  |D x => numerator x
  end
with    
denominator (r:rational) : nat :=
  match r with
  |one => 1
  |N x => denominator x
  |D x => denominator x + numerator x
  end.
...
Compute denominator ( N (D (D ( N one)))).
...
Inductive inf_tree (T:Type) :=
  leaf | node  of T & (nat -> inf_tree T). 
...
Fixpoint has_zero (t: @inf_tree nat) n {struct t} : bool :=
  ...
  | leaf => false
  | node x f => (x == 0) ||
               check_subtrees (fun m => has_zero (f m) n) n
  end.

Eval compute in 
  has_zero (node 2 (fun x => if x == 3 then node 0 (fun x => leaf _) else leaf _)) 3.
